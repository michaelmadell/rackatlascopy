...
describe('location/floor/room routes', () => {
  ...
  describe('building routes', () => {
    it('creates a building under a location, and a floor under that building', async () => {
      const location = await app.inject({
        method: 'POST',
        url: `/tenant/${TENANT_ID}/location`,
        headers: AUTH,
      ...
      const building = await app.inject({
        method: 'POST',
        url: `/tenant/${TENANT_ID}/building`,
        headers: AUTH,
        payload: { name: 'Building A', reference: 'A', locationId }
      })
      expect(building.statusCode).toBe(200)
      const buildingId = building.json().data._id
      expect(building.json().data.locationId).toBe(locationId)

      const floor = await app.inject({
        method: 'POST',
        url: `/tenant/${TENANT_ID}/floor`,
        headers: AUTH,
        payload: { name: 'Ground Floor', level: 0, locationId, buildingId }
      })
      expect(floor.statusCode).toBe(200)
      expect(floor.json().data.buildingId).toBe(buildingId)

      const listed = await app.inject({
        method: 'GET',
        url: `/tenant/${TENANT_ID}/building?locationId=${locationId}`,
        headers: AUTH
      })
      expect(listed.json().data.totalDocs).toBe(1)
      expect(listed.json().data.docs[0].name).toBe('Building A')

      const floorsInBuilding = await app.inject({
        method: 'GET',
        url: `/tenant/${TENANT_ID}/floor?buildingId=${buildingId}`,
        headers: AUTH
      })
      expect(floorsInBuilding.json().data.totalDocs).toBe(1)
      expect(floorsInBuilding.json().data.docs[0]._id).toBe(floor.json().data._id)
    })

    it('does not return buildings scoped to a different tenant', async () => {
      await app.inject({
        method: 'POST',
        url: '/tenant/tenant-2/building',
        headers: AUTH,
        payload: { name: 'Other Building', locationId: 'loc-x' }
      })
      const res = await app.inject({ method: 'GET', url: `/tenant/${TENANT_ID}/building`, headers: AUTH })
      expect(res.json().data.totalDocs).toBe(0)
    })

    it('GET/PATCH/DELETE on a building reject a mismatched tenantId in the URL', async () => {
      const created = await app.inject({
        method: 'POST',
        url: `/tenant/${TENANT_ID}/building`,
        headers: AUTH,
        payload: { name: 'Building A', locationId: 'loc-x' }
      })
      const buildingId = created.json().data._id

      const getWrongTenant = await app.inject({
        method: 'GET',
        url: `/tenant/tenant-2/building/${buildingId}`,
        headers: AUTH
      })
      expect(getWrongTenant.statusCode).toBe(404)
      ...
      const stillThere = await app.inject({
        method: 'GET',
        url: `/tenant/${TENANT_ID}/building/${buildingId}`,
        headers: AUTH
      })
      expect(stillThere.statusCode).toBe(200)
      expect(stillThere.json().data.name).toBe('Building A')
    })
  })
